...
<body>
	<section class="Hui-article-box">
		<div id="iframe_box" class="Hui-article">
			<nav class="breadcrumb">
				...
				审核管理 <span class="c-gray en">&gt;</span> 已审核 <a
					...
					<table
						...
						<tbody>
							<c:forEach items="${articles}" var="article">
								<tr class="text-c" id="${article.id}">
									<td>1</td>
									<td>${article.articleTitle }</td>
									<td><a _href="">${article.articleAuthor }</a></td>
									<td>${article.articleKey }</td>
									<td>${article.typeName }</td>
									<td>${article.lstUpdateOn }</td>
									<td class="td-manage"><a style="text-decoration: none"
										_href="${ctx}/manager/detailArticle.htm?articleId=${article.id}&isAudit=1"
	...
	$(document).ready(function(){
		var serviceScriptArr = ["${ctx}/resources/js/commonDataTable.js","${ctx}/resources/js/auditedArticlesM.js"];
